...
{//evaluated in anti clockwise direction from la to ra.tange: 0-2PI
	if (ra > PI * 2)
		ra -= PI * 2, la -= PI * 2;
	if (ra < la) ra += 2 * PI;//all circle will be from 0 to 2*PI...ra is in 3rd and 4th quadrants.
...
int main ()
{
	...
	REP(_t, T)
	{
		...
		REP(i, n)
		{
			...
			{//now with j
				...
				if (!(dst + A[i].r <= A[j].r + eps) && !(dst + A[j].r <= A[i].r + eps) && !(dst >= A[i].r + A[j].r - eps))//if they intersects
				{
					...
					ld dt = acos((sqr(A[i].r) + sqr(dst) - sqr(A[j].r)) / (2 * dst * A[i].r));//finds delta of angle(law of cosines)
				...
				if (dst + A[i].r < A[j].r + eps)//if circle i is inside circle j
					add_arc(0, 2 * PI, i);// remove entire circle
			}
			//intersection with boundary
			//left side
			if (fabs(A[i].o.x) <= A[i].r - eps)
			{
				ld x = sqrt (sqr(A[i].r) - sqr(A[i].o.x));// using pythagoras theorem..Point where la starts is (0,x). ra starts at (0,-x).
		...
		REP(i, n)//for each circle sorts all ends
			sort (B[i].begin(), B[i].end(), &cmp);// all angles range from 0 to 2PI where la(start of arc) < ra(end of arc)
		ld ans = 0;
		REP(j, n)
		{
			ld cur = 0;
